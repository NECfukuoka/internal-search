...
GSI.Searcher = L.Class.extend( {
	...
	searchStart : function( q )
	{
		...
		this.clearSearch();
		this.dialog.show();
		this.addresAjax = this.searchCSIS( q, "","" );
		this.chimeiAjax = this.searchChimei( q, "","" );
	},
	/**
	 * 住所検索を実行
	 */
	searchChimei : function (searchWord)
	{
		var parameter = {
				"q" : searchWord
		};
		return $.ajax({
			type: "GET",
			url:CONFIG.SERVERAPI.CHIMEI_SEARCH,
			data: parameter,
			dataType: "json",
			timeout: 30000,
			success: L.bind( this.setChimeiResult, this ),
			error:function(){
			}
		});
		
	},
	setChimeiResult : function(json) {
		this.dialog.setChimeiResult(json);
	},
	/**
	 * 住所検索を実行
	 */
	searchCSIS : function (searchWord)
	{
		var parameter = {
				"q" : searchWord
		};
		return $.ajax({
			type: "GET",
			url:"/GsiJsLibrary/address-search/address-search.php",
			data: parameter,
			dataType: "json",
			timeout: 30000,
			success: L.bind( this.setCSISResult, this ),
			error:function(){
			}
		});
		
	},
	setCSISResult : function(json) {
		// 緯度経度から市区町村コードを取得
		$.each(json,function() {
			...
			$.ajax({
				...
				success: function(data) {
					that.properties.addressCode = data.results.muniCd;
				},
				error : function(){}
			});
		});
		this.dialog.setCSISResult(json);
	},
...
GSI.Footer = L.Class.extend( {
	 ...
	 * 取得した標高値を処理します。
	 */
	getElevationResult : function (json)
	{
		var data = json;
	...
	execRefresh : function (lon, lat)
	{
		...
		this.ajaxElevation = $.ajax({
			...
			success:  L.Util.bind( this.getElevationResult, this )
		});
	}
} );
...
GSI.SearchResultDialog = GSI.Dialog.extend( {
	...
	setChimeiResult : function(results) {
		this.chimeiResult = results;
		this.showResult();		
	},
	setCSISResult : function(results) {
